...
popX <- model.matrix(~age + gender, data = acs_nc %>% count(age, gender, wt = N))[,-1]
...
## result
res <- cbind(popY,
  predict(fit, newdata = popX, counts = popN),
  as.vector(crossprod(predict(fit2, newdata = popX),  popN / sum(popN))),
  colMeans(Y)
)
